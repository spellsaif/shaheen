...
## 🏗️ Architecture & TurboModule (JSI) Integration
...
```
   ...
   └─────────────────────────────┬─────────────────────────────┘
                                 │
                                 │  [TurboModule JSI Invocation]
                                 ▼
   ┌───────────────────────────────────────────────────────────┐
   │         React Native New Architecture JSI Engine          │
   │  - Maps JavaScript parameters directly to native types    │
   │  - Instantly transitions context to native platform layer │
   └─────────────────────────────┬─────────────────────────────┘
                                 │
                                 │  [Platform Thread Offload]
                                 ▼
   ┌───────────────────────────────────────────────────────────┐
   │             Native Background Thread (Java/GCD)           │
   │  - Spawns a background thread to prevent JS blocking      │
   │  - Calls native Rust FFI over standard C-ABI boundary     │
   └─────────────────────────────┬─────────────────────────────┘
                                 │
                                 │  [C-ABI FFI Boundary]
                                 ▼
...
2. **JSI Invocation**: JavaScript calls the autogenerated TurboModule JSI wrapper. Control instantly transitions to the native platform layer.
3. **Intent Launch**: The system Intent wakes up the **Solana Mobile Wallet** app on the user's phone.
4. **Rust Handshake**: The native background thread opens a local loopback TCP connection to the wallet's WebSocket server and calls Rust. The Rust engine exchanges P-256 public keys via plaintext `HELLO` frames.
...
7. **Return**: The signed transaction is received by Rust, decoded, and both the signature and the updated signed transaction hex string are passed back to the native module. The platform background thread resolves the JS Promise.

---

## 🛠️ The Tech Stack: Why Rust + TurboModules (JSI)?

A common question is: *Why do we use the React Native New Architecture (TurboModules) and Rust instead of writing the entire module in JavaScript or C++?*

Writing a secure cryptographic socket engine in JavaScript is slow, and doing so in C++ is highly complex and error-prone:
...
* **The Hybrid Synergy**: 
  * We use **TurboModules (JSI)** for what they do best: direct, high-performance typed interactions with the Hermes JavaScript VM that are compatible with Bridgeless Mode.
...
* **TurboModule JSI Pass-through**: Ephemeral parameters (like transaction hex strings and auth tokens) are passed directly across the boundary using React Native's New Architecture JSI codegen. We avoid heavy JSON serialization overhead on the JS side, bypassing standard React Native bridge bottlenecks.
